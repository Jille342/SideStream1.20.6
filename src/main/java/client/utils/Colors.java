...
public class Colors implements MCUtil {
    ...
    public static Color getHealthColor(float health, float maxHealth) {
        ...
        Color[] colors =
            {new Color(108, 0, 0), new Color(255, 51, 0), Color.GREEN};
        ...
        if (fractions != null) {
            if (colors != null) {
                if (fractions.length == colors.length) {
                    ...
                    float[] range = new float[]{fractions[indicies[0]],
    ...
    public static int getTeamColor(PlayerEntity entityIn) {
        int i = -1;
        if (Objects.requireNonNull(entityIn.getDisplayName()).getString()
            .equalsIgnoreCase("§f[§cR§f]§c" + entityIn.getName())) {
            i = Colors.getColor(new Color(255, 60, 60));
        } else if (entityIn.getDisplayName().getString()
            .equalsIgnoreCase("§f[§9B§f]§9" + entityIn.getName())) {
            i = Colors.getColor(new Color(60, 60, 255));
        } else if (entityIn.getDisplayName().getString()
            .equalsIgnoreCase("§f[§eY§f]§e" + entityIn.getName())) {
            i = Colors.getColor(new Color(255, 255, 60));
        } else if (entityIn.getDisplayName().getString()
            .equalsIgnoreCase("§f[§aG§f]§a" + entityIn.getName())) {
    ...
    public static Color rainbow(long time, float count, float fade) {
        ...
        long color = Long.parseLong(
            Integer.toHexString(
                Integer.valueOf(Color.HSBtoRGB(hue, 1.0F, 1.0F)).intValue()),
            16);
        Color c = new Color((int) color);
        return new Color(c.getRed() / 255.0F * fade,
            c.getGreen() / 255.0F * fade, c.getBlue() / 255.0F * fade,
            c.getAlpha() / 255.0F);
    }
    ...
    public static int rainbow(int delay, float saturation, float brightness) {
        ...
        rainbow %= 360.0D;
        return Color
            .getHSBColor((float) (rainbow / 360.0D), saturation, brightness)
            .getRGB();
    }

    public static Color getRainbowColor(double boost, double offset, double p) {
        double rainbow = Math.ceil(
            ((System.currentTimeMillis() * boost) + (offset * 2) * 2) / 5);
        rainbow %= p;
    ...
    public static Color getSuperCoolColor(double boost, double offset, double p) {
        ...
        Color[] colors = {new Color(55, 33, 255), new Color(255, 70, 90),
            new Color(50, 150, 255), new Color(55, 33, 255)};
        double progress = Math.ceil(
            ((System.currentTimeMillis() * boost) + (offset * 2) * 2) / 5);
        progress %= p;
        return RenderingUtils
            .blendColors(fractions, colors, (float) (progress / p)).brighter();
    }
}
