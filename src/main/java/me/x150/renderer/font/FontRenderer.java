...
/**
 * A simple font renderer. Supports multiple fonts, passed in as {@link Font}
 * instances.
 * <h2>Constructor</h2>
 * {@link FontRenderer#FontRenderer(Font[], float)} takes in a {@link Font}
 * array with the fonts to use. All fonts in this array will be asked, in order,
 * if they have a glyph corresponding to the one being drawn. If none of them
 * have it, the missing "square" is drawn.
 */
public class FontRenderer implements Closeable {
     ...
     *
     * @param fonts  The fonts to use. The font renderer will go over each font in
     *               this array, search for the glyph, and render it if found. If
     *               no font has the specified glyph, it will draw the missing font
     *               symbol.
     * @param sizePx The size of the font in minecraft pixel units. One pixel unit
     *               = `guiScale` pixels
     */
    ...
    private GlyphMap generateMap(char from, char to) {
        GlyphMap gm = new GlyphMap(from, to, this.fonts,
            RendererUtils.randomIdentifier());
        maps.add(gm);
        return gm;
    }
    ...
    private Glyph locateGlyph0(char glyph) {
        ...
        int base = floorNearestMulN(glyph, BLOCK_SIZE); // if not, generate a
        // new page and return
        // the generated glyph
    ...
    public void drawString(MatrixStack stack, String s, float x, float y,
        ...
        for (int i = 0; i < chars.length; i++) {
            ...
            if (c == '§') {
                ...
                yOffset +=
                    getStringHeight(s.substring(lineStart, i)) * scaleMul;
                xOffset = 0;
                lineStart = i + 1;
                continue;
            }
            Glyph glyph = locateGlyph1(c);
            if (glyph.value() != ' ') { // we only need to really draw the glyph if it's not blank,
                // otherwise we can just skip its width and that'll be it
                Identifier i1 = glyph.owner().bindToTexture;
                DrawEntry entry =
                    new DrawEntry(xOffset, yOffset, r2, g2, b2, glyph);
                GLYPH_PAGE_CACHE
                    .computeIfAbsent(i1, integer -> new ObjectArrayList<>())
                    .add(entry);
            }
            xOffset += glyph.width();
        }
        for (Identifier identifier : GLYPH_PAGE_CACHE.keySet()) {
            ...
            for (DrawEntry object : objects) {
                ...
                bb.vertex(mat, xo + 0, yo + h, 0).texture(u1, v2)
                    .color(cr, cg, cb, a).next();
                bb.vertex(mat, xo + w, yo + h, 0).texture(u2, v2)
                    .color(cr, cg, cb, a).next();
                bb.vertex(mat, xo + w, yo + 0, 0).texture(u2, v1)
                    .color(cr, cg, cb, a).next();
                bb.vertex(mat, xo + 0, yo + 0, 0).texture(u1, v1)
                    .color(cr, cg, cb, a).next();
            }
            BufferUtils.draw(bb);
        }
     ...
     */
    public void drawCenteredString(MatrixStack stack, String s, float x,
    ...
    /**
     * Calculates the height of the string, if it were drawn on the screen. This
     * is necessary, because the fonts in this FontRenderer might have a
     * different height for each char.
     *
     * @param text The text to simulate
        ...
        for (char c1 : c) {
            if (c1 == '\n') {
                if (currentLine == 0) {
                    ...
                    currentLine =
                        locateGlyph1(' ').height() / (float) this.scaleMul;
                }
            ...
            Glyph glyph = locateGlyph1(c1);
            currentLine =
                Math.max(glyph.height() / (float) this.scaleMul, currentLine);
        }
        return currentLine + previous;
    }

    /**
     * Clears all glyph maps, and unlinks them. The font can continue to be
     * used, but it will have to regenerate the maps.
     */
    @Override
    public void close() {
    ...
    record DrawEntry(float atX, float atY, float r, float g, float b,
                     Glyph toDraw) {}
}
