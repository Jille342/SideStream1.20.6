...
class GlyphMap {
    ...
    GlyphMap(char fromIncl, char toExcl, Font[] font, Identifier bindToTexture) {
    ...
    public void generate() {
        ...
        int charsVert = (int) (Math.ceil(Math.sqrt(range)) * 1.5); // double as
        // many
        // chars
        // wide as
        // high
        ...
        while (generatedChars <= range) {
            ...
            Rectangle2D stringBounds =
                font.getStringBounds(String.valueOf(currentChar), frc);
            ...
            if (charNX >= charsVert) {
                currentX = 0;
                currentY += currentRowMaxY + PADDING; // add height of highest
                // glyph, and reset
                charNX = 0;
                currentRowMaxY = 0;
            }
            currentRowMaxY = Math.max(currentRowMaxY, height); // calculate the
            // highest glyph
            // in this row
            glyphs1.add(new Glyph(currentX, currentY, width, height,
                currentChar, this));
            currentX += width + PADDING;
            charNX++;
        }
        BufferedImage bi = new BufferedImage(Math.max(maxX + PADDING, 1),
            Math.max(maxY + PADDING, 1), BufferedImage.TYPE_INT_ARGB);
        width = bi.getWidth();
        height = bi.getHeight();
        ...
        for (Glyph glyph : glyphs1) {
            ...
            g2d.drawString(String.valueOf(glyph.value()), glyph.u(),
                glyph.v() + fontMetrics.getAscent());
    ...
    private Font getFontForGlyph(char c) {
        ...
        return this.font[0]; // no font can display it, so it doesn't matter
        // which one we pick; it'll always be missing
    }

    public void destroy() {
        MinecraftClient.getInstance().getTextureManager()
            .destroyTexture(this.bindToTexture);
        this.glyphs.clear();
        this.width = -1;
        this.height = -1;
        generated = false;
