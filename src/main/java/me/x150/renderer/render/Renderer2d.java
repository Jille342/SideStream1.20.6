...
/**
 * The rendering class for the 2nd dimension, used in the hud renderer or in
 * screens
 */
...
public class Renderer2d {
    ...
    private static final float[][] roundedCache =
        new float[][]{new float[3], new float[3], new float[3], new float[3],};
    ...
    static void beginScissor(double x, double y, double endX, double endY) {
        ...
        int ay =
            (int) ((client.getWindow().getScaledHeight() - (y + height)) * d);
        RenderSystem.enableScissor((int) (x * d), ay, (int) (width * d),
            (int) (height * d));
    }
     ...
     * </p>
     * <p>
     * Does the binding for you, call this instead of
     * {@link #renderTexture(MatrixStack, double, double, double, double)} or
     * {@link #renderTexture(MatrixStack, double, double, double, double, float, float, double, double, double, double)}
     * for ease of use
     * </p>
     *
     ...
     */
    public static void renderTexture(MatrixStack matrices, Identifier texture,
     ...
     * </p>
     * <p>
     * Make sure to link your texture using
     * {@link RenderSystem#setShaderTexture(int, Identifier)} before using this
     * </p>
     *
     * @param matrices The context MatrixStack
     * @param x        The X coordinate
     ...
     * @param height   The height of the texture
     */
    public static void renderTexture(MatrixStack matrices, double x, double y,
                                     double width, double height) {
        renderTexture(matrices, x, y, width, height, 0, 0, width, height, width,
            height);
    }
     ...
     */
    public static void renderTexture(MatrixStack matrices, double x0, double y0,
                                     double width, double height, float u, float v, double regionWidth,
                                     double regionHeight, double textureWidth, double textureHeight) {
        double x1 = x0 + width;
        double y1 = y0 + height;
        double z = 0;
        renderTexturedQuad(matrices.peek().getPositionMatrix(), x0, x1, y0, y1,
            z, (u + 0.0F) / (float) textureWidth,
            (u + (float) regionWidth) / (float) textureWidth,
            (v + 0.0F) / (float) textureHeight,
            (v + (float) regionHeight) / (float) textureHeight);
    }

    private static void renderTexturedQuad(Matrix4f matrix, double x0,
                                           double x1, double y0, double y1, double z, float u0, float u1, float v0,
                                           float v1) {
     ...
     * @param segments    Number of segments that should be rendered. Increasing will
     *                    decrease speed, decreasing with decrease quality
     */
    public static void renderCircle(MatrixStack matrices, Color circleColor,
                                    double originX, double originY, double rad,
                                    @Range(from = 4, to = 360) int segments) {
        renderEllipse(matrices, circleColor, originX, originY, rad, rad,
            segments);
    }
     ...
     * @param segments     How many segments to use to render the ellipse (less = more
     *                     performance, more = higher quality ellipse)
     */
    public static void renderEllipse(MatrixStack matrices, Color ellipseColor,
                                     double originX, double originY, double radX, double radY,
                                     @Range(from = 4, to = 360) int segments) {
        ...
        for (int i = 0; i < 360; i += Math.min(360d / segments, 360 - i)) {
            ...
            buffer
                .vertex(matrix, (float) (originX + sin), (float) (originY + cos),
                    0)
                .color(colorFloat[0], colorFloat[1], colorFloat[2],
                    colorFloat[3])
                .next();
        }
        setupRender();
    ...
    static float[] getColor(Color c) {
        return new float[]{c.getRed() / 255f, c.getGreen() / 255f,
            c.getBlue() / 255f, transformColor(c.getAlpha() / 255f)};
    }
     ...
     *                     decrease speed, decreasing with decrease quality
     */
    public static void renderEllipseOutline(MatrixStack matrices,
                                            Color ellipseColor, double originX, double originY, double radX,
                                            double radY, @Range(from = 0, to = Long.MAX_VALUE) double width,
                                            @Range(from = 0, to = Long.MAX_VALUE) double height,
                                            @Range(from = 4, to = 360) int segments) {
        segments = MathHelper.clamp(segments, 4, 360);
        ...
        for (int i = 0; i <= segments; i++) {
            ...
            buffer
                ...
                .next();
            buffer
                .vertex(matrix, (float) (originX + sin1),
                    (float) (originY + cos1), 0)
                .color(colorFloat[0], colorFloat[1], colorFloat[2],
                    colorFloat[3])
                .next();
        }
     ...
     */
    public static void renderQuad(MatrixStack matrices, Color color, double x1,
     ...
     */
    public static void renderRoundedQuad(MatrixStack stack, Color c, double x,
                                         double y, double x1, double y1, float rad, float samples) {
     ...
     * @param samples  Samples per corner
     */
    public static void renderRoundedQuad(MatrixStack matrices, Color c,
                                         double fromX, double fromY, double toX, double toY, float radTL,
                                         float radTR, float radBL, float radBR, float samples) {
        Matrix4f matrix = matrices.peek().getPositionMatrix();
        ...
        renderRoundedQuadInternal(matrix, r, g, b, a, (float) fromX,
            (float) fromY, (float) toX, (float) toY, radTL, radTR, radBL, radBR,
            samples);
        endRender();
    }
    ...
    private static void renderRoundedQuadInternal(Matrix4f matrix, float cr,
                                                  ...
                                                  float toY, float radC1, float radC2, float radC3, float radC4,
                                                  float samples) {
        BufferBuilder bufferBuilder = Tessellator.getInstance().getBuffer();
        bufferBuilder.begin(DrawMode.TRIANGLE_FAN,
        ...
        for (int i = 0; i < 4; i++) {
            ...
            for (float r = i * 90f; r <= (i + 1) * 90f; r += 90 / samples) {
                ...
                bufferBuilder
                    .vertex(matrix, current[0] + sin, current[1] + cos, 0.0F)
     ...
     */
    public static void renderRoundedOutline(MatrixStack matrices, Color c,
                                            double fromX, double fromY, double toX, double toY, float rad,
                                            float width, float samples) {
        renderRoundedOutline(matrices, c, fromX, fromY, toX, toY, rad, rad, rad,
            rad, width, samples);
    }
    ...
    public static void renderRoundedOutline(MatrixStack matrices, Color c,
                                            double fromX, double fromY, double toX, double toY, float radTL,
                                            float radTR, float radBL, float radBR, float outlineWidth,
                                            float samples) {
        Matrix4f matrix = matrices.peek().getPositionMatrix();
        float[] color1 = getColor(c);
        float r = color1[0];
        ...
        renderRoundedOutlineInternal(matrix, r, g, b, a, (float) fromX,
            (float) fromY, (float) toX, (float) toY, radTL, radTR, radBL, radBR,
            outlineWidth, samples);
        endRender();
    }

    private static void renderRoundedOutlineInternal(Matrix4f matrix, float cr,
                                                     float cg, float cb, float ca, float fromX, float fromY, float toX,
                                                     float toY, float radC1, float radC2, float radC3, float radC4,
                                                     float width, float samples) {
        BufferBuilder bufferBuilder = Tessellator.getInstance().getBuffer();
        bufferBuilder.begin(DrawMode.TRIANGLE_STRIP,
            VertexFormats.POSITION_COLOR);
        ...
        for (int i = 0; i < 4; i++) {
            ...
            for (float r = i * 90f; r <= (i + 1) * 90f; r += 90 / samples) {
                ...
                bufferBuilder
                    ...
                    .color(cr, cg, cb, ca).next();
                bufferBuilder
                    .vertex(matrix, current[0] + sin + sin1 * width,
                        current[1] + cos + cos1 * width, 0.0F)
                    .color(cr, cg, cb, ca).next();
            }
        }
        ...
        float rad = current[2];
        bufferBuilder.vertex(matrix, current[0], current[1] + rad, 0.0F)
            .color(cr, cg, cb, ca).next();
        bufferBuilder.vertex(matrix, current[0], current[1] + rad + width, 0.0F)
            .color(cr, cg, cb, ca).next();
        BufferUtils.draw(bufferBuilder);
    }
     ...
     */
    public static void renderLine(MatrixStack stack, Color color, double x,
                                  double y, double x1, double y1) {
        float[] colorFloat =
            Colors.intArrayToFloatArray(Colors.ARGBIntToRGBA(color.getRGB()));
