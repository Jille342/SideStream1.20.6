...
public class MSAAFramebuffer extends Framebuffer {
    ...
    private MSAAFramebuffer(int samples) {
        ...
        if (samples < MIN_SAMPLES || samples > MAX_SAMPLES) {
            throw new IllegalArgumentException(String.format(
                "The number of samples should be >= %s and <= %s.", MIN_SAMPLES,
                MAX_SAMPLES));
        }
     ...
     * </p>
     *
     * @param samples    The desired amount of samples to be used. While everything up
     *                   to (and including) MAX_SAMPLES is accepted, anything above 16
     *                   is generally overkill.
     * @param drawAction The runnable that gets executed within the framebuffer. Render
     *                   your things there.
     */
    public static void use(int samples, Runnable drawAction) {
        use(samples, MinecraftClient.getInstance().getFramebuffer(),
            drawAction);
    }
     ...
     * </p>
     * <p>
     * If you do not know what you're doing, use {@link #use(int, Runnable)}
     * instead
     * </p>
     *
     * @param samples    The desired amount of samples to be used
     * @param mainBuffer The main framebuffer to read and write to once the buffer
     *                   finishes
     * @param drawAction The runnable that gets executed within the framebuffer. Render
     *                   your things there.
     */
    public static void use(int samples, Framebuffer mainBuffer,
                           Runnable drawAction) {
        ...
        GlStateManager._glBindFramebuffer(GL30C.GL_DRAW_FRAMEBUFFER,
            msaaBuffer.fbo);
        GlStateManager._glBlitFrameBuffer(0, 0, msaaBuffer.textureWidth,
            msaaBuffer.textureHeight, 0, 0, msaaBuffer.textureWidth,
            msaaBuffer.textureHeight, GL30C.GL_COLOR_BUFFER_BIT,
            GL30C.GL_LINEAR);
    ...
    public static MSAAFramebuffer getInstance(int samples) {
        return INSTANCES.computeIfAbsent(samples,
            x -> new MSAAFramebuffer(samples));
    }
    ...
    public void initFbo(int width, int height, boolean getError) {
        ...
        GlStateManager._glBindRenderbuffer(GL30C.GL_RENDERBUFFER,
            this.rboColor);
        GL30.glRenderbufferStorageMultisample(GL30C.GL_RENDERBUFFER, samples,
            GL30C.GL_RGBA8, width, height);
        ...
        GlStateManager._glBindRenderbuffer(GL30C.GL_RENDERBUFFER,
            this.rboDepth);
        GL30.glRenderbufferStorageMultisample(GL30C.GL_RENDERBUFFER, samples,
            GL30C.GL_DEPTH_COMPONENT, width, height);
        GlStateManager._glBindRenderbuffer(GL30C.GL_RENDERBUFFER, 0);

        GL30.glFramebufferRenderbuffer(GL30C.GL_FRAMEBUFFER,
            GL30C.GL_COLOR_ATTACHMENT0, GL30C.GL_RENDERBUFFER, this.rboColor);
        GL30.glFramebufferRenderbuffer(GL30C.GL_FRAMEBUFFER,
            GL30C.GL_DEPTH_ATTACHMENT, GL30C.GL_RENDERBUFFER, this.rboDepth);
