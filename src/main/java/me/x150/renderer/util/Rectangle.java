...
public class Rectangle {
    ...
    public String toString() {
        return String.format(Locale.ENGLISH, "%s{x=%f, y=%f, x1=%f, y1=%f}",
            getClass().getSimpleName(), this.x, this.y, this.x1, this.y1);
    }

    /**
     * Checks if {@code this} rectangle overlaps with {@code that} rectangle. An
     * overlap is defined as any point of either rectangle being within the
     * other.
     *
    ...
    public boolean overlaps(Rectangle that) {
        return this.x1 >= that.x && this.y1 >= that.y && this.x <= that.x1
            && this.y <= that.y1;
    }
}
