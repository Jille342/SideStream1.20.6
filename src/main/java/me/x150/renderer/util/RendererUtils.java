...
public class RendererUtils {
    ...
    public static int lerp(int from, int to, double delta) {
        return (int) Math
            .floor(from + (to - from) * MathHelper.clamp(delta, 0, 1));
    }
     ...
     * </p>
     * <p>
     * Any of the components can be set to -1 to keep them from the original
     * color
     * </p>
     *
    ...
    public static Color modify(Color original, int redOverwrite,
                               int greenOverwrite, int blueOverwrite, int alphaOverwrite) {
    ...
    public static Vec3d translateVec3dWithMatrixStack(MatrixStack stack,
                                                      Vec3d in) {
    ...
    /**
     * <p>
     * Registers a BufferedImage as Identifier, to be used in future render
     * calls
     * </p>
     * <p>
     * <strong>WARNING:</strong> This will wait for the main tick thread to
     * register the texture, keep in mind that the texture will not be available
     * instantly
     * </p>
     * <p>
     * <strong>WARNING 2:</strong> This will throw an exception when called when
     * the OpenGL context is not yet made
     * </p>
     *
     * @param i  The identifier to register the texture under
     * @param bi The BufferedImage holding the texture
     */
    public static void registerBufferedImageTexture(Identifier i,
                                                    BufferedImage bi) {
        try {
            ...
            ByteBuffer data =
                BufferUtils.createByteBuffer(bytes.length).put(bytes);
            data.flip();
            NativeImageBackedTexture tex =
                new NativeImageBackedTexture(NativeImage.read(data));
            MinecraftClient.getInstance().execute(() -> MinecraftClient
                .getInstance().getTextureManager().registerTexture(i, tex));
    ...
    public static MatrixStack getEmptyMatrixStack() {
        ...
        empty.loadIdentity(); // reset top to identity, in case someone modified
        // it
        return empty;
    }

    /**
     * Gets the position of the crosshair of the player, transformed into world
     * space
     *
     * @return The position of the crosshair of the player, transformed into
     * world space
     */
    @Contract("-> new")
    ...
    /**
     * Transforms an input position into a (x, y, d) coordinate, transformed to
     * screen space. d specifies the far plane of the position, and can be used
     * to check if the position is on screen. Use
     * {@link #screenSpaceCoordinateIsVisible(Vec3d)}.
     * Example:
     *
     * <pre>
     * {@code
    ...
    public static Vec3d worldSpaceToScreenSpace(Vec3d pos) {
        ...
        Vector4f transformedCoordinates =
        ...
        matrixProj.mul(matrixModel).project(transformedCoordinates.x(),
            transformedCoordinates.y(), transformedCoordinates.z(),
            lastViewport, target);
    ...
    /**
     * Converts a (x, y, d) screen space coordinate back into a world space
     * coordinate. Example:
     *
     * <pre>
     * {@code
     ...
     * // Ray-cast from near to far to get block or entity at (100, 100) screen
     * // space
     * }
     * </pre>
     *
    ...
    public static Vec3d screenSpaceToWorldSpace(double x, double y, double d) {
        ...
        matrixProj.mul(matrixModel).mul(lastWorldSpaceMatrix).unproject(
            (float) x / displayWidth * lastViewport[2],
            (float) (displayHeight - y) / displayHeight * lastViewport[3],
            (float) d, lastViewport, target);
    ...
    private static String randomString(int length) {
        return IntStream.range(0, length)
            .mapToObj(operand -> String
                .valueOf((char) RND.nextInt(RND_START, RND_END + 1)))
