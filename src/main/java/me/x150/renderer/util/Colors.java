...
/**
 * Helper for colors, specifically for parsing hex colors into rgb(a) components
 * and converting rgb(a) components into hex colors
 */
public class Colors {
    /**
     * <p>
     ...
     */
    public static int ARGBToInt(@Range(from = 0, to = 255) int r,
                                @Range(from = 0, to = 255) int g, @Range(from = 0, to = 255) int b,
                                @Range(from = 0, to = 255) int a) {
        Preconditions.checkArgument(validateColorRange(r),
            "Expected r to be 0-255, received " + r);
        Preconditions.checkArgument(validateColorRange(g),
            "Expected g to be 0-255, received " + g);
        Preconditions.checkArgument(validateColorRange(b),
            "Expected b to be 0-255, received " + b);
        Preconditions.checkArgument(validateColorRange(a),
            "Expected a to be 0-255, received " + a);
     ...
     */
    public static int RGBAToInt(@Range(from = 0, to = 255) int r,
                                @Range(from = 0, to = 255) int g, @Range(from = 0, to = 255) int b,
                                @Range(from = 0, to = 255) int a) {
        Preconditions.checkArgument(validateColorRange(r),
            "Expected r to be 0-255, received " + r);
     ...
     * @return A length 4 array containing the R, G, B and A component of the
     * color
     */
    ...
    public static float[] intArrayToFloatArray(int[] in) {
        Preconditions.checkArgument(in.length == 4,
            "Expected int[] of size 4, got " + in.length);
        for (int i = 0; i < in.length; i++) {
            Preconditions.checkArgument(validateColorRange(in[i]),
                "Expected in[" + i + "] to be 0-255, got " + in[i]);
        }
        return new float[]{in[0] / 255f, in[1] / 255f, in[2] / 255f,
            in[3] / 255f};
    }
}
